...
def update(inf_file) -> int:
    ...
    try: 
        ...
        if('Failed' in result.stdout): raise
    except: 
        err: str = get_err(result.stdout)
        with open(OUTFILE, mode='a') as fhandle:  fhandle.write(f'DRIVER CONFIG FILE: {inf_file}{chr(0x0a)}{err}{chr(0x0a)*2}')
